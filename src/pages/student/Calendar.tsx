...
const StudentCalendar: React.FC = () => {
  ...
  // 1. Initial check (Conforme lock + Fetch Academic Years)
  useEffect(() => {
    const initializeCalendar = async () => {
      if (!user?.id) return

      try {
        const [{ data: profileData }, { data: settings }, { data: years }] = await Promise.all([
          supabase
            .from('profiles')
        ...
        const userConformeYear = enrolledYear || ''
        ...
        console.error('Error initializing student calendar:', err)
      }
    }
    initializeCalendar()
  }, [user])
